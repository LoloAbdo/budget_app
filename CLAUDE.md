...
Current version: **2.12.0**. Releases: v1.0.0 (portable only), v1.0.1 (+ installer),
...
v2.9.0 (post recurring on tab open + rollover alert fix),
v2.10.0 (label-fit buttons + per-user column widths),
v2.11.0 (dashboard spending insights + subscription detector),
v2.12.0 (Amazon shopping price tracker + user-guide PDF + settings-tab scroll fix).

## Architecture
- `main.py` — entry point + data-path logic.
- `database/schema.py` — `DatabaseManager` (all SQL, parameterized) + idempotent migrations run on every init (latest: v1.0.10 adds per-user `font_scale` + `accent`; v1.0.9 adds a per-account `currency` column backfilled from the owner's home currency plus the `fx_rates` cache table; v1.0.8 adds an `is_active` flag to recurring_transactions; v1.0.7 adds a nullable `end_date`; v1.0.6 adds a per-user `theme` column; v1.0.5 adds performance indexes). Every mutating method calls `_log()` to append an entry to the `audit_log` table (append-only activity trail, exported via `ImportExportService.export_audit_log_csv`); internal side-effects (balance updates, watch-cache refreshes) and password hashes are deliberately not logged. Money is rounded to cents on write via `_money()`, and balance updates use SQL `ROUND(...,2)`. Analytics helpers include `get_net_worth_history()` (dashboard net-worth trend, reconstructed by unwinding monthly flow — no balance-history table), `get_budget_alerts()` (categories ≥90% of monthly budget), `get_spending_insights()` (dashboard month-over-month anomaly/trend cards: overall + per-category swings vs the trailing 3-month average, warnings-first, capped at 4) and `get_detected_subscriptions()` (recurring-charge detection straight from transaction history — groups by a normalized merchant key, keeps groups with a regular cadence + steady amount; powers the Subscriptions panel). The Shopping price tracker lives in `watched_items` + `price_history` (created in `SCHEMA_SQL`): `add_watched_item` freezes `start_price`, `update_item_price` sets `current_price`/`last_checked` + appends history + freezes start on first success, `mark_item_blocked` flags a bot-walled fetch, and `get_price_alerts()` returns items whose current price fell below start (powers the dashboard **Price Drops** card).
- `services/` — `auth` (login/register + one-time password recovery codes: bcrypt-hashed in the `recovery_codes` table, generated in Settings ▸ Security, consumed by the login screen's "Forgot password?" page; the reset flow returns one generic error for every failure cause so e-mails can't be probed), `backup`, `import_export`, `recurring`, `market` (stocks/crypto), `update` (GitHub releases check), `fx` (exchange-rate cache refresh; also exports the `CURRENCIES` picker list), `credential_store` (optional "Remember me" login: email + DPAPI-encrypted password in QSettings, never plaintext; degrades to email-only where DPAPI is unavailable), `shopping` (`shopping_service.py`: Amazon price scraping for the Shopping tab — pure `urllib` + `beautifulsoup4`, browser-like UA; `fetch_item`/`fetch_wishlist` touch the network, the `parse_*`/`asin_from_url` helpers are pure + unit-tested on saved HTML; a CAPTCHA/bot page raises `FetchBlocked`. Scraping Amazon violates their ToS and blocks intermittently — best-effort by design). `RecurringService.forecast()` projects the combined balance forward from recurring income/expenses in the home currency (powers the Forecast panel; transfers excluded since they don't change net worth).
- `views/` — PyQt6 panels; `main_window.py` wires the sidebar + signals. `theme.py` holds QSS + chart colors (incl. the virtual `"auto"` theme — resolved to dark/light via `resolve_theme()` from the OS color scheme; MainWindow re-applies live on `colorSchemeChanged`). `i18n.py` is the translation layer. `activity_view.py` is a read-only in-app viewer for the `audit_log`. `shopping_view.py` is the Shopping price-tracker tab (table + `QThreadPool` fetch workers, add-product/add-wishlist dialogs; `refresh_on_open()` re-checks only items stale >1h on tab open, throttled; emits `prices_changed` to refresh the dashboard alert). `search_dialog.py` is the global Ctrl+F search (debounced, jumps to the Transactions panel via `TransactionsView.apply_global_search`). `winutil.py` darkens native Windows title bars to match dark themes (app-wide event filter installed in `main.py`).
...
- `CHANGELOG.md` (+ the French `CHANGELOG.fr.md`) is bundled into the frozen build (`--add-data "CHANGELOG.md;."` and `--add-data "CHANGELOG.fr.md;."` in both build scripts + `release.yml`) and rendered read-only in **Settings ▸ About ▸ What's New** so users see what changed after updating (`settings_view.load_changelog()` → `QTextBrowser.setMarkdown`; resolves next to source or in `sys._MEIPASS`). `load_changelog()` follows the app language: it loads `CHANGELOG.<lang>.md` when present (currently only `fr`) and falls back to English otherwise — so **every release entry must be added to both `CHANGELOG.md` and `CHANGELOG.fr.md`** (a test enforces the current version appears in each). It's still the single in-app source — no separate copy baked into code.
